...
#define MAX_PARTY_MONS 6 //gPartiesCount[B_TRAINER_PLAYER] - 1

void ShowSelectMovePokemonSummaryScreen_SwSh(struct Pokemon *mons, u8 monIndex, void (*callback)(void), u16 newMove)
{
    ShowPokemonSummaryScreen_SwSh(SUMMARY_MODE_SELECT_MOVE, mons, monIndex, MAX_PARTY_MONS - 1, callback);
...
static bool32 HasAnyRelearnableMoves(s32 state)
{
...
static void PssScrollEnd(u8 taskId)
{
    if (sMonSummaryScreen->mode == SUMMARY_MODE_SELECT_MOVE)
    {
        if (sMonSummaryScreen->currPageIndex == PSS_PAGE_CONTEST_MOVES)
        {
            u16 move = sMonSummaryScreen->firstMoveIndex == MAX_MON_MOVES
...
static enum Type SummaryScreen_GetDynamicMoveType(struct Pokemon *mon, u16 move, enum Type type)
{
...
static void CB2_ReturnToSummaryScreenFromNamingScreen(void)
{
    ...
    ShowPokemonSummaryScreen(SUMMARY_MODE_NORMAL, gPlayerParty, gSpecialVar_0x8004, MAX_PARTY_MONS - 1, gInitialSummaryScreenCallback);
}
