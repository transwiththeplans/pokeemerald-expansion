...
SINGLE_BATTLE_TEST("Aerilate turns a Normal-type move into Flying-type move (Trait)")
{
    GIVEN {
        PLAYER(SPECIES_MEGANIUM);
        OPPONENT(SPECIES_SALAMENCE) { Ability(ABILITY_MOXIE); Innates(ABILITY_AERILATE); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Aerilate can not turn certain moves into Flying type moves (Trait)")
{
    u32 move;
    PARAMETRIZE { move = MOVE_WEATHER_BALL; }
    // PARAMETRIZE { move = MOVE_NATURAL_GIFT; } TODO: handle this case via Skill Swap
...
SINGLE_BATTLE_TEST("Aerilate boosts power of affected moves by 20% (Gen7+) or 30% (Gen1-6) (Trait)", s16 damage)
{
    u32 ability, genConfig;
    PARAMETRIZE { ability = ABILITY_LIGHT_METAL;   genConfig = GEN_7; }
    PARAMETRIZE { ability = ABILITY_LIGHT_METAL;   genConfig = GEN_6; }
...
SINGLE_BATTLE_TEST("Aerilate doesn't affect Weather Ball's type (Trait)", s16 damage)
{
    u32 move1, ability;
    PARAMETRIZE { move1 = MOVE_CELEBRATE; ability = ABILITY_LIGHT_METAL; }
    PARAMETRIZE { move1 = MOVE_SUNNY_DAY; ability = ABILITY_LIGHT_METAL; }
    PARAMETRIZE { move1 = MOVE_CELEBRATE; ability = ABILITY_AERILATE; }
    PARAMETRIZE { move1 = MOVE_SUNNY_DAY; ability = ABILITY_AERILATE; }
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_WEATHER_BALL) == EFFECT_WEATHER_BALL);
        ASSUME(GetMoveType(MOVE_WEATHER_BALL) == TYPE_NORMAL);
        ...
        TURN { MOVE(opponent, MOVE_CELEBRATE); MOVE(player, move1); }
        TURN { MOVE(player, MOVE_WEATHER_BALL); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Aerilate doesn't affect Natural Gift's type (Trait)")
{
    u16 ability;
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Item(ITEM_PERSIM_BERRY); Ability(ABILITY_SHADOW_TAG); Innates(ability); }
        OPPONENT(SPECIES_SALAMENCE) { Item(ITEM_SALAMENCITE); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Aerilate doesn't affect Judgment / Techno Blast / Multi-Attack's type (Trait)")
{
    u16 move, item;
    PARAMETRIZE { move = MOVE_JUDGMENT; item = ITEM_ZAP_PLATE; }
    PARAMETRIZE { move = MOVE_TECHNO_BLAST; item = ITEM_SHOCK_DRIVE; }
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { Item(item); Ability(ABILITY_SHADOW_TAG); Innates(ABILITY_AERILATE); }
        OPPONENT(SPECIES_DIGLETT);
    } WHEN {
...
SINGLE_BATTLE_TEST("Aerilate doesn't affect Hidden Power's type (Trait)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_HIDDEN_POWER) == EFFECT_HIDDEN_POWER);
        ASSUME(gTypesInfo[TYPE_ELECTRIC].isHiddenPowerType == TRUE);
...
TO_DO_BATTLE_TEST("Aerilate doesn't override Electrify (Gen7+) (Trait)"); // No mon with Aerilate exists in Gen8+, but probably behaves similar to Pixilate, which does.
TO_DO_BATTLE_TEST("Aerilate doesn't override Ion Deluge (Gen7+) (Trait)"); // Ion Deluge doesn't exist in Gen 8+, but we probably could assume it behaves similar to under Electrify. TODO: Test by hacking SV.
TO_DO_BATTLE_TEST("Aerilate overrides Electrify (Gen6) (Trait)")
TO_DO_BATTLE_TEST("Aerilate overrides Ion Deluge (Gen6) (Trait)")
TO_DO_BATTLE_TEST("Aerilate doesn't affect Tera Starstorm's type (Trait)");
TO_DO_BATTLE_TEST("Aerilate doesn't affect Max Strike's type (Trait)");
TO_DO_BATTLE_TEST("Aerilate doesn't affect Terrain Pulse's type (Trait)");
TO_DO_BATTLE_TEST("Aerilate doesn't affect damaging Z-Move types (Trait)");
TO_DO_BATTLE_TEST("(DYNAMAX) Aerilate turns Max Strike into Max Airstream (Trait)"); // All other -ate abilities do this, so interpolating this as no Aerilate mon is available in a Dynamax game
