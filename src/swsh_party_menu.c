...
static bool8 ShowPartyMenu(void)
{
    ...
    case 8:
        ...
        else if (sPartyMenuInternal->switchCounter < 0)
        {
            ExitPartyMenu();
            return TRUE;
        }
    ...
    case 11:
        ...
        {
            if (CreateMonSpritesGfxManager(MON_SPR_GFX_MANAGER_A, MON_SPR_GFX_MODE_NORMAL) == NULL)
            {
                ExitPartyMenu();
                return TRUE;
            }
...
static bool8 DecompressGraphics(void)
{
    ...
    case 0:
        if (sPartyBgGfxTilemap == NULL)
        {
            ...
            if (sPartyBgGfxTilemap == NULL)
            {
                sPartyMenuInternal->switchCounter = -1;
                return FALSE;
            }
        }
        else
...
static void CreatePartyMonIconSpriteParameterized(u16 species, u32 pid, bool32 isEgg, struct PartyMenuBox *menuBox, u8 priority)
{
    ...
    {
        if(isEgg)
            menuBox->monSpriteId = CreateMonIcon(SPECIES_EGG, SpriteCB_MonIcon, menuBox->spriteCoords[0], menuBox->spriteCoords[1], 4, pid);
        else
...
static void UpdatePartyMonHeldItemSprite(struct Pokemon *mon, struct PartyMenuBox *menuBox)
{
    ...
    else
    {
        ...
        if (!IsValidSpriteId(menuBox->itemSpriteId) && GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE)
        {
            menuBox->itemSpriteId = CreateSprite(&sSpriteTemplate_HeldItem, menuBox->spriteCoords[2], menuBox->spriteCoords[3], 1);
            if (IsValidSpriteId(menuBox->itemSpriteId))
                gSprites[menuBox->itemSpriteId].subpriority = 2;
...
static void CreateItemIconSprite(struct PartyMenuBox *menuBox, u8 slot, u16 item)
{
    ...
    if (IsValidSpriteId(sItemIconSpriteId) && gSprites[sItemIconSpriteId].inUse)
    {
...
static void CreateHoverSprite(struct PartyMenuBox *menuBox, u8 slot)
{
    ...
    else
    {
        ...
        if (IsValidSpriteId(sHoverCursorSpriteId) && gSprites[sHoverCursorSpriteId].inUse)
        {
...
void Task_InnateUnlock(u8 taskId)
{
    static const u8 askText[] = _("Would you like to unlock {STR_VAR_1}'s\ninnate?");
    static const u8 doneText[] = _("{STR_VAR_1}'s innate was\nunlocked!{PAUSE_UNTIL_PRESS}");
    s16 *data = gTasks[taskId].data;
    ...
    case 5:
        tAbilityNum = TRUE;
        SetMonData(&gPlayerParty[tMonId], MON_DATA_INNATE_UNLOCKED, &tAbilityNum);
        RemoveBagItem(gSpecialVar_ItemId, 1);
        gTasks[taskId].func = Task_ClosePartyMenu;
        break;
    }
...
void ItemUseCB_InnateCandy(u8 taskId, TaskFunc task)
{
    ...
    tAbilityNum = GetMonData(&gPlayerParty[tMonId], MON_DATA_INNATE_UNLOCKED, NULL);
    SetWordTaskArg(taskId, tOldFunc, (uintptr_t)(gTasks[taskId].func));
    gTasks[taskId].func = Task_InnateUnlock;
}
...
static void DisplayPartyPokemonDataForFormChange(u8 slot)
{
    ...
    targetSpecies = GetFormChangeTargetSpecies(mon, FORM_CHANGE_ITEM_USE, 0); //ToCheck
