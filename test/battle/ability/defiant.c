...
SINGLE_BATTLE_TEST("Defiant doesn't activate after Sticky Web lowers Speed if Court Changed (Gen8) (Trait)")
{
    GIVEN {
        WITH_CONFIG(GEN_CONFIG_DEFIANT_STICKY_WEB, GEN_8);
        ASSUME(GetMoveEffect(MOVE_GROWL) == EFFECT_ATTACK_DOWN);
...
SINGLE_BATTLE_TEST("Defiant activates after Sticky Web lowers Speed if Court Changed (Gen9) (Trait)")
{
    GIVEN {
        WITH_CONFIG(GEN_CONFIG_DEFIANT_STICKY_WEB, GEN_9);
        ASSUME(GetMoveEffect(MOVE_GROWL) == EFFECT_ATTACK_DOWN);
...
SINGLE_BATTLE_TEST("Defiant doesn't display ability popup when already at Maximum Attack (Trait)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TICKLE) == EFFECT_TICKLE);
        ASSUME(GetMoveEffect(MOVE_BELLY_DRUM) == EFFECT_BELLY_DRUM);
