...
SINGLE_BATTLE_TEST("Minior Core doesn't transform into Minior Meteor on switch-in if it has 1/2 or less health (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET) { HP(1); }
        OPPONENT(SPECIES_MINIOR_CORE) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_SHIELDS_DOWN); HP(50); MaxHP(100); }
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); SEND_OUT(opponent, 1); }
    } SCENE {
        NONE_OF {
...
SINGLE_BATTLE_TEST("Minior Core transforms into Minior Meteor on switch-in if it has more than 1/2 health (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET) { HP(1); }
        OPPONENT(SPECIES_MINIOR_CORE) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_SHIELDS_DOWN); HP(51); MaxHP(101); }
    } WHEN {
        TURN { MOVE(player, MOVE_SCRATCH); SEND_OUT(opponent, 1); }
    } SCENE {
        ABILITY_POPUP(opponent, ABILITY_SHIELDS_DOWN);
...
SINGLE_BATTLE_TEST("Shields Down protects Minior Meteor from status conditions (Multi)")
{
    ...
    GIVEN {
        ...
        ASSUME(GetMoveNonVolatileStatus(MOVE_WILL_O_WISP) == MOVE_EFFECT_BURN);
        PLAYER(SPECIES_WYNAUT);
        OPPONENT(species) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_SHIELDS_DOWN); HP(hp); MaxHP(300); }
    } WHEN {
        TURN { MOVE(player, MOVE_WILL_O_WISP); }
    } SCENE {
        if (species == SPECIES_MINIOR_METEOR)
