...
DOUBLE_BATTLE_TEST("Aura Break inverts Fairy Aura's effect (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_XERNEAS) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_FAIRY_AURA); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_ZYGARDE_50) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_AURA_BREAK); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_PLAY_ROUGH, target:opponentLeft); }
        TURN { MOVE(playerLeft, MOVE_PLAY_ROUGH, target:opponentLeft); SWITCH(playerRight, 2); }
        TURN { MOVE(playerLeft, MOVE_PLAY_ROUGH, target:opponentLeft); SWITCH(opponentRight, 2); }
...
DOUBLE_BATTLE_TEST("Aura Break inverts Dark Aura's effect (Multi)")
{
    s16 damage[3];
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_YVELTAL) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_DARK_AURA); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_ZYGARDE_50) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_AURA_BREAK); }
    } WHEN {
...
DOUBLE_BATTLE_TEST("Aura Break ignores Mold Breaker abilities (Multi)")
{
    s16 damage[6];
    u32 species = SPECIES_WOBBUFFET, ability = ABILITY_NONE, ability2 = ABILITY_NONE;

    PARAMETRIZE { species = SPECIES_WOBBUFFET, ability = ABILITY_SHADOW_TAG; ability2 = ABILITY_TELEPATHY; }
    PARAMETRIZE { species = SPECIES_CRANIDOS, ability = ABILITY_MOLD_BREAKER; ability2 = ABILITY_SHEER_FORCE; }
    PARAMETRIZE { species = SPECIES_ZEKROM, ability = ABILITY_TERAVOLT; ability2 = ABILITY_LIGHT_METAL; }
    PARAMETRIZE { species = SPECIES_RESHIRAM, ability = ABILITY_TURBOBLAZE; ability2 = ABILITY_LIGHT_METAL;}

    GIVEN {
        PLAYER(species) { Ability(ability2); Innates(ability); Level(50); }
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_YVELTAL) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_DARK_AURA); }
        PLAYER(SPECIES_XERNEAS) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_FAIRY_AURA); }
        OPPONENT(SPECIES_WOBBUFFET);
