...
#if MAX_MON_TRAITS > 1
DOUBLE_BATTLE_TEST("Dark Aura increases the power of all Dark-type attacks by 33% (Traits)")
{
    s16 damage[8];

    GIVEN {
        PLAYER(SPECIES_YVELTAL) { Ability(ABILITY_LIGHT_METAL); }
        PLAYER(SPECIES_YVELTAL) { Ability(ABILITY_LIGHT_METAL); }
        PLAYER(SPECIES_YVELTAL) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_DARK_AURA);}
        OPPONENT(SPECIES_YVELTAL) { Ability(ABILITY_LIGHT_METAL); }
        OPPONENT(SPECIES_YVELTAL) { Ability(ABILITY_LIGHT_METAL); }
    } WHEN {
        TURN {
            MOVE(playerLeft, MOVE_BITE, target:opponentLeft, secondaryEffect:FALSE);
        ...
        // Turn 2
        ABILITY_POPUP(playerRight, ABILITY_DARK_AURA);
        ...
        EXPECT_MUL_EQ(damage[0], UQ_4_12(1.33), damage[4]);
        EXPECT_MUL_EQ(damage[1], UQ_4_12(1.33), damage[5]);
        EXPECT_MUL_EQ(damage[2], UQ_4_12(1.33), damage[6]);
        EXPECT_MUL_EQ(damage[3], UQ_4_12(1.33), damage[7]);
    }
}

DOUBLE_BATTLE_TEST("Dark Aura's effect doesn't stack multiple times (Traits)")
{
    s16 damage[6];
