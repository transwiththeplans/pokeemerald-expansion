...
SINGLE_BATTLE_TEST("Mycelium Might will respect the speed if both battlers have the ability (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_TOEDSCOOL) { Speed(100); Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_MYCELIUM_MIGHT); }
        OPPONENT(SPECIES_TOEDSCOOL) { Speed(1); Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_MYCELIUM_MIGHT); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_WILL_O_WISP); MOVE(player, MOVE_WILL_O_WISP); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_WILL_O_WISP, player);
...
SINGLE_BATTLE_TEST("Mycelium Might ignores opposing abilities (Multi)")
{
    GIVEN {
        PLAYER(SPECIES_TOEDSCOOL) { Speed(100); Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_MYCELIUM_MIGHT); }
        OPPONENT(SPECIES_BELDUM) { Speed(1); Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_CLEAR_BODY);}
    } WHEN {
        TURN { MOVE(opponent, MOVE_CELEBRATE); MOVE(player, MOVE_SCREECH); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_CELEBRATE, opponent);
