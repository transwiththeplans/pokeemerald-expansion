...
DOUBLE_BATTLE_TEST("Battery increases Sp. Attack damage of allies by ~30% (Multi)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_CHARJABUG) { Speed(1); Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_BATTERY); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(2); Moves(MOVE_SHOCK_WAVE); }
        PLAYER(SPECIES_CHARJABUG) { Speed(1); Ability(ABILITY_LIGHT_METAL); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(4); Moves(MOVE_CELEBRATE, MOVE_GASTRO_ACID); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(3); }
    } WHEN {
        TURN { MOVE(playerRight, MOVE_SHOCK_WAVE, target: opponentLeft); }
        TURN { SWITCH(playerLeft, 2); MOVE(playerRight, MOVE_SHOCK_WAVE, target: opponentLeft);}
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_SHOCK_WAVE, playerRight);
        HP_BAR(opponentLeft, captureDamage: &damage[0]);
