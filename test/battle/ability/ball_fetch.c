...
WILD_BATTLE_TEST("Ball Fetch causes the Pokémon to pick up the last failed Ball at the end of the turn (Trait)")
{
    u32 item = 0;
...
WILD_BATTLE_TEST("Ball Fetch doesn't trigger if the Pokémon is already holding an item (Trait)")
{
    u32 item = 0;
...
WILD_BATTLE_TEST("Ball Fetch only picks up the first failed ball, once per battle (Trait)")
{
    u32 item = 0;
    u32 item2 = 0;
...
SINGLE_BATTLE_TEST("Ball Fetch doesn't trigger in Trainer Battles (Trait)")
{
    u32 item = 0;
...
TO_DO_BATTLE_TEST("Ball Fetch doesn't trigger in Max Raid Battles (Trait)");
