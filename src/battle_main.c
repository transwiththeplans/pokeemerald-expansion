...
u8 CreateNPCTrainerPartyFromTrainer(struct Pokemon *party, const struct Trainer *trainer, bool32 firstTrainer, u32 battleTypeFlags, u16 trainerId)
{
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        u8 specialTrainerPool = GetSpecialTrainerPool(trainerId);
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            u32 fixedOtId = 0;
            u32 abilityNum = 0;
            u16 species = GetTrainerSpeciesFromPool(partyData[monIndex].species, partyData[monIndex].lvl, trainerId, trainer->trainerClass, party, i, specialTrainerPool);
            ...
            if (partyData[monIndex].ev != NULL&& !hasChangedSpecies)
            {
            ...
            if (partyData[monIndex].ability != ABILITY_NONE && !hasChangedSpecies)
            {
                const struct SpeciesInfo *speciesInfo = &gSpeciesInfo[species];
            ...
            if (partyData[monIndex].innateUnlocked)
            {
                bool32 data = TRUE;
                SetMonData(&party[i], MON_DATA_INNATE_UNLOCKED, &data);
            }
...
static u8 CreateNPCTrainerParty(struct Pokemon *party, u16 trainerNum, bool8 firstTrainer)
{
    ...
    if (GetTrainerStructFromId(trainerNum)->overrideTrainer)
    {
        ...
        retVal = CreateNPCTrainerPartyFromTrainer(party, (const struct Trainer *)(&tempTrainer), firstTrainer, gBattleTypeFlags, TRAINER_ID_DYNAMIC);
    }
    else
    {
...
s32 GetBattleMovePriority(u32 battler, u32 move)
{
    ...
    if (SearchTraits(battlerTraits, ABILITY_RAPID_FIRE) && IsHealingMove(move))
        priority += 3;
...
enum Type TrySetAteType(u32 move, u32 battlerAtk, enum Ability attackerAbility)
{
    ...
    else if (BattlerHasTrait(battlerAtk, ABILITY_VANTABLACK))
        ateType = TYPE_DARK;
    else if (BattlerHasTrait(battlerAtk, ABILITY_FOSSILIZE))
        ateType = TYPE_ROCK;
    else if (BattlerHasTrait(battlerAtk, ABILITY_EMANATE))
        ateType = TYPE_PSYCHIC;
    else if (BattlerHasTrait(battlerAtk, ABILITY_METAL_MORPH))
        ateType = TYPE_STEEL;
    else if (BattlerHasTrait(battlerAtk, ABILITY_IMMOLATE))
        ateType = TYPE_FIRE;    
