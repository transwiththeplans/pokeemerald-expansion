...
static bool32 ShouldSwitchIfHasBadOdds(u32 battler)
{
    ...
    if (maxDamageTaken > gBattleMons[battler].hp
        && !(gItemsInfo[gBattleMons[battler].item].holdEffect == HOLD_EFFECT_FOCUS_SASH || (!HasMoldBreakerTypeAbility(opposingBattler) && GetConfig(CONFIG_STURDY) >= GEN_5 && AI_BATTLER_HAS_TRAIT(battler, ABILITY_STURDY))))
    {
        getsOneShot = TRUE;
    }
...
static bool32 ShouldSwitchIfBadlyStatused(u32 battler)
{
    ...
    if (gAiThinkingStruct->aiFlags[battler] & AI_FLAG_SMART_SWITCHING)
    {
        ...
        // Secondary Damage
        if (!AI_BATTLER_HAS_TRAIT(battler, ABILITY_MAGIC_GUARD) && !AI_BATTLER_HAS_TRAIT(battler, ABILITY_IMPENETRABLE)
...
bool32 IsMonGrounded(enum HoldEffect heldItemEffect, enum Ability ability, enum Type type1, enum Type type2, u16 species)
{
    ...
    if (type1 == TYPE_FLYING || type2 == TYPE_FLYING || (ability == ABILITY_LEVITATE || SpeciesHasInnate(species, ABILITY_LEVITATE)) || (ability == ABILITY_EELEVATE || SpeciesHasInnate(species, ABILITY_EELEVATE))
...
static u32 GetSwitchinHazardsDamage(u32 battler, struct BattlePokemon *battleMon)
{
    ...
    if ((ability != ABILITY_MAGIC_GUARD  && !SpeciesHasInnate(species, ABILITY_MAGIC_GUARD)  && 
         ability != ABILITY_IMPENETRABLE && !SpeciesHasInnate(species, ABILITY_IMPENETRABLE) && 
         ability != ABILITY_MOUNTAINEER  && !SpeciesHasInnate(species, ABILITY_MOUNTAINEER)  && 
         ability != ABILITY_BLOW_AWAY    && !SpeciesHasInnate(species, ABILITY_BLOW_AWAY))   ||
        (heldItemEffect == HOLD_EFFECT_HEAVY_DUTY_BOOTS &&
        !((gFieldStatuses & STATUS_FIELD_MAGIC_ROOM) || (ability == ABILITY_LEVITATE || SpeciesHasInnate(species, ABILITY_LEVITATE) || ability == ABILITY_EELEVATE || SpeciesHasInnate(species, ABILITY_EELEVATE)))))
    {
        // Stealth Rock
        if (IsHazardOnSide(side, HAZARDS_STEALTH_ROCK) && heldItemEffect != HOLD_EFFECT_HEAVY_DUTY_BOOTS && !SpeciesHasInnate(species, ABILITY_BLOW_AWAY) && !SpeciesHasInnate(species, ABILITY_MOUNTAINEER))
            hazardDamage += GetStealthHazardDamageByTypesAndHP(TYPE_SIDE_HAZARD_POINTED_STONES, defType1, defType2, battleMon->maxHP);
        // G-Max Steelsurge
        if (IsHazardOnSide(side, HAZARDS_STEELSURGE) && heldItemEffect != HOLD_EFFECT_HEAVY_DUTY_BOOTS && !SpeciesHasInnate(species, ABILITY_BLOW_AWAY) && !SpeciesHasInnate(species, ABILITY_MOUNTAINEER))
...
static s32 GetSwitchinWeatherImpact(void)
{
    ...
    {
        // Damage
        if (holdEffect != HOLD_EFFECT_SAFETY_GOGGLES && (ability != ABILITY_MAGIC_GUARD && !SpeciesHasInnate(species, ABILITY_MAGIC_GUARD) && ability != ABILITY_IMPENETRABLE && !SpeciesHasInnate(species, ABILITY_IMPENETRABLE)) && (ability != ABILITY_OVERCOAT && !SpeciesHasInnate(species, ABILITY_OVERCOAT)))
        {
...
static u32 GetSwitchinRecurringDamage(void)
{
    ...
    // Items
    if ((ability != ABILITY_MAGIC_GUARD && !SpeciesHasInnate(species, ABILITY_MAGIC_GUARD)) && (ability != ABILITY_IMPENETRABLE && !SpeciesHasInnate(species, ABILITY_IMPENETRABLE)) && (ability != ABILITY_KLUTZ && !SpeciesHasInnate(species, ABILITY_KLUTZ)))
    {
        if (holdEffect == HOLD_EFFECT_BLACK_SLUDGE && gAiLogicData->switchinCandidate.battleMon.types[0] != TYPE_POISON && gAiLogicData->switchinCandidate.battleMon.types[1] != TYPE_POISON)
        {
...
static u32 GetSwitchinStatusDamage(u32 battler)
{
    ...
    if ((status != 0) && (ability != ABILITY_MAGIC_GUARD && !SpeciesHasInnate(species, ABILITY_MAGIC_GUARD) && ability != ABILITY_IMPENETRABLE && !SpeciesHasInnate(species, ABILITY_IMPENETRABLE)))
    {
    ...
    if (tSpikesLayers != 0 && (defType1 != TYPE_POISON && defType2 != TYPE_POISON
        && (ability != ABILITY_IMMUNITY    && !SpeciesHasInnate(species, ABILITY_IMMUNITY)) 
        && (ability != ABILITY_MOUNTAINEER && !SpeciesHasInnate(species, ABILITY_MOUNTAINEER))
        && (ability != ABILITY_POISON_HEAL && !SpeciesHasInnate(species, ABILITY_POISON_HEAL))
        && (ability != ABILITY_BLOW_AWAY   && !SpeciesHasInnate(species, ABILITY_BLOW_AWAY))
        && status == 0
        && !(heldItemEffect == HOLD_EFFECT_HEAVY_DUTY_BOOTS
...
static u32 GetSwitchinHitsToKO(s32 damageTaken, u32 battler)
{
    ...
    while (currentHP > 0)
    {
        ...
        if (damageTaken >= maxHP && startingHP == maxHP && (heldItemEffect == HOLD_EFFECT_FOCUS_SASH || (!opponentCanBreakMold && GetConfig(CONFIG_STURDY) >= GEN_5 && (gAiLogicData->switchinCandidate.battleMon.ability == ABILITY_STURDY || SpeciesHasInnate(gAiLogicData->switchinCandidate.battleMon.species, ABILITY_STURDY)))) && hitsToKO < 1)
            currentHP = 1;
