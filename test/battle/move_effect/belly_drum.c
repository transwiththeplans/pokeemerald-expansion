...
SINGLE_BATTLE_TEST("Belly Drum fails if the user's Attack is already at +6, even with Contrary (Trait)")
{
    KNOWN_FAILING;
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_CHARM) == EFFECT_ATTACK_DOWN_2);
        PLAYER(SPECIES_SERPERIOR) { Ability(ABILITY_SHADOW_TAG); Innates(ABILITY_CONTRARY); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_CHARM); }
        TURN { MOVE(opponent, MOVE_CHARM); }
...
SINGLE_BATTLE_TEST("Belly Drum deducts HP if the user has Contrary and is at -6 (Trait)")
{
    KNOWN_FAILING;
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SWORDS_DANCE) == EFFECT_ATTACK_UP_2);
